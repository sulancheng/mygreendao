    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Column<Boolean> column6 = new Column<>("勾选2", "isCheck", new TextImageDrawFormat<Boolean>(size, size, TextImageDrawFormat.LEFT, 10) {
            ...
            protected int getResourceID(Boolean isCheck, String value, int position) {
                ...
                return Color.parseColor("#fff555");
            }
        });
...
//        table.getConfig().setShowYSequence(false);
        tableData.setOnItemClickListener(new TableData.OnItemClickListener() {
            ...
            public void onClick(Column column, String value, Object o, int col, int row) {
                ...
                List<UserInfo> t = tableData.getT();
                UserInfo userInfo = t.get(row);
                userInfo.setCheck(!userInfo.isCheck());
                Log.i("nishishenm"," ---"+userInfo.toString());
                Toast.makeText(ParseModeActivity.this, "列:" + col + " 行：" + row + "数据：" + value, Toast.LENGTH_SHORT).show();
//                ICellBackgroundFormat<CellInfo> backgroundFormat = new BaseCellBackgroundFormat<CellInfo>() {
//                    @Override
//                    public int getBackGroundColor(CellInfo cellInfo) {
//                        if (cellInfo.row  == row&&cellInfo.col==col) {
//                            return ContextCompat.getColor(ParseModeActivity.this, R.color.arc21);
//                        }
...
//                table.invalidate();
        ...
        table.getConfig().setTableTitleStyle(new FontStyle(this, 15, getResources().getColor(R.color.colorAccent)).setAlign(Paint.Align.CENTER));
        ICellBackgroundFormat<CellInfo> backgroundFormat = new BaseCellBackgroundFormat<CellInfo>() {
            @Override
            public int getBackGroundColor(CellInfo cellInfo) {
                if("childData.child".equals(cellInfo.column.getFieldName())){
                    List<UserInfo> t = tableData.getT();
                    UserInfo userInfo = t.get(cellInfo.row);
                    if (userInfo.isCheck()){
                        return ContextCompat.getColor(ParseModeActivity.this, R.color.arc22);
                    }
                }
//                if (cellInfo.row % 2 == 0) {
//                    return ContextCompat.getColor(ParseModeActivity.this, R.color.arc22);
